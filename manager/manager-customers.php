...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if ($_POST["username"] != ""){
    ...
    $sql .= " `customer_name` LIKE '%$username%' AND";
  }
  ...
  if ( $identity == "Unknown" ) $sql .= " `identity` is NULL AND";
  else if ( $identity != "All" ) $sql .= " `identity` = '$identity' AND"; 
...
<body class="manager-page">
  ...
  <main class="main-body-with-side-bar">
    <h2 class="title">Search Customers</h2>

    <hr />

    <form action="manager-customers.php" method="post">
    <div class="filter-container">
      <div class="filter-list">
        <div class="filter-list-item">
          <label>ID: </label>
          <input id="filter-id" style="width: 8em;" class="input-box" type="number" min="1" placeholder="Customer ID" value="" name="customer_id"/>
        </div>
        <div class="filter-list-item">
      ...
      <tbody>
        ...
        foreach ($result as $row){
          ...
          if ( $cnt > ($page - 1) * 10 && $cnt <= $page * 10 ){
            ...
            echo"
            <tr>
              <td><a href='manager-customer-detail.php?customer_id=$cid'>$cid</a></td>
              <td>$cname</td>
              <td>$nickname</td>
              <td>$phone</td>
              <td>$identity</td>
